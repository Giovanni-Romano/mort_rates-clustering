...
source("C:/Users/RomanoGi/Desktop/Bocconi/Ricerca/mort_rates-clustering/funzioni.R")

Y_og <- rbind(ita_man = log(Y_ita_man / N_ita_man)[1:87, 1],
           swe_man = log(Y_swe_man / N_swe_man)[1:87, 1],
           uk_man = log(Y_uk_man / N_uk_man)[1:87, 1],
           us_man = log(Y_us_man / N_us_man)[1:87, 1])
rownames(Y_og) <- substr(rownames(Y_og), 1, 2)

perm <- enumerate.permutations(4)
Y_list <- apply(perm, 1, function(x) Y_og[x, ], simplify = F)
names(Y_list) <- apply(perm, 1, 
                       function(x) paste(rownames(Y_og)[x], collapse = "_"),
                       simplify = F)


cl <- makeCluster(8, 
                  outfile = "Progress.txt")
registerDoParallel(cl)


out <- foreach(x=seq_along(Y_list)) %dopar% {
  set.seed(1)
  ...
  for (d in 2:n_iter){ # Ciclo sulle iterazioni
    ...
    if ((d %% 500) == 0) {cat("Job", x, "iter", d, 
                              difftime(Sys.time(), inizio, units = "m"), "\n")}
  ...
  exec_time <- difftime(fine, inizio, units = "m")
  ...
  params <- list(T_final = T_final,
                 ...
                 n_iter = n_iter,
                 eps_tau = eps_tau,
                 hyperpar = list(sigma2_i = sigma2_i,
                                 m0 = m0, s02 = s02,
                                 A_tau = A_tau,
  ...
  functions <- list(GGup_iid = GaussGaussUpdate_iid,
                    up_alpha = up_alpha,
                    up_beta = up_beta,
                    up_gamma_i = up_gamma_i,
                    up_label_i = up_label_i,
                    up_theta_t = up_theta_t,
                    up_var.RWM = up_var.RWM)
...
save.image("res/sim_only_infant/simplest_model/comparison_different_data_order2.RData")